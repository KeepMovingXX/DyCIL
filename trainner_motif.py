...
class NC_Motif_Trainer(object):
    ...
    def train(self, epoch, data):
        ...
        causal_embedding_list, conf_embedding_list, env_kl_loss= self.model(data['edge_index_list'], self.x[:self.len_train], self.len_train)
        ...
        for t in range(self.len_train):
            ...
            causal_pred = self.cal_pred(causal_embedding, self.model.cs_decoder, self.node_mask[t].to(self.args.device))
            causal_loss = torch.cat([causal_loss, criterion(causal_pred, data['label_list'][t].squeeze().to(self.args.device)).unsqueeze(0)])
            # causal intervention
            for times in range(self.args.intervention_times):
                n_s = np.random.randint(self.len_train)
                conf_z = conf_embedding_list[n_s]
                conf_pred = self.cal_pred(conf_z, self.model.ss_decoder, self.node_mask[t].to(self.args.device))
                s1 = np.random.randint(len(conf_pred))
                conf_pred_s = torch.sigmoid(conf_pred[s1]).detach()
                conf = conf_pred_s * causal_pred
                env_loss = torch.cat([env_loss, criterion(conf, data['label_list'][t].squeeze().to(self.args.device)).unsqueeze(0)])
        ...
        for t in range(self.len):
            ...
            acc = self.predict(z, self.model.cs_decoder, self.node_mask[t].to(self.args.device), data['label_list'][t])
            if t < self.len_train:
